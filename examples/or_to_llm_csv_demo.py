...
def make_hybrid_vm_agent(initial_samples: dict = None, promised_lead_time: int = 0,
    ...
    system = (
        ...
        "- Exact dates are provided; leverage world knowledge for seasonality and real events.\n"
        ...
        "- Orders occasionally (rarely) get lost. If a shipment is overdue by 2+ periods beyond expected arrival in CONCLUDED history, "
        "adjust your safety stock accordingly.\n"
        "\n"
        "=== OR BASELINE (CAPPED) ===\n"
        "- The OR agent sets base_stock = μ̂ + z*·σ̂ (z* = Φ⁻¹(q), q = profit/(profit+holding_cost)).\n"
        "- It always applies the capped policy: order = min(base_stock − pipeline_inventory, μ̂/(1+L) + Φ⁻¹(0.95)·σ̂/√(1+L)).\n"
        "- OR assumes the promised lead time and cannot see news, lost orders, or actual arrivals. Its recommendation is a data-driven baseline, not a mandate.\n"
        "\n"
        ...
        "If an expected arrival is missing from a CONCLUDED period by 2+ periods, decide whether lead time increased or goods were lost.\n"
        "- Use the SKU description plus exact date (no ongoing news feed) to confirm sustained demand mean/variance shifts before reacting.\n"
        "- Historical samples provide initial intuition, but demand can change suddenly; confirm shifts quickly when evidence appears.\n"
        "- Apply world knowledge whenever the SKU description and exact date imply known seasonal or cultural effects.\n"
        "\n"
    )
