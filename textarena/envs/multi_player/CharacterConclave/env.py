...
class CharacterConclaveEnv(ta.Env):
    ...
    def __init__(self, num_players: int = 5, character_budget: int = 1_000):
        """
        ...
        self.state = ta.State(
            ...
            max_turns=None,  # No strict turn limit; ends when budgets are out + voting done
        )
    ...
    def reset(self, seed: Optional[int] = None) -> None:
        ...
        Args:
            seed (Optional[int]): If needed, seed any RNG for reproducibility.
        """
        if seed is not None:
            random.seed(seed)
    ...
    def _check_and_evaluate_outcome(self):
        if len(self.state.game_state["votes"]) == self.state.num_players:
            ...
            self.state.set_winners(
                ...
                reason=f"Player(s) {','.join(winner_ids)} received the most votes ({max_votes})."
            )
