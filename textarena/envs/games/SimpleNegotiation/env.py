...
from textarena.envs.games.SimpleNegotiation.renderer import create_board_str
...
class SimpleNegotiationEnv(ta.Env):
    ...
    def __init__(self, max_turns: Optional[int] = 10):
        ...
        self.action_space = lambda player_id: re.compile(r"\[Accept\]|\[Deny\]|\[Offer:?\s*(?:I\s+(?:give|offer)\s+)?([^\[\]]+?)\s*\.*\]", re.IGNORECASE | re.DOTALL)
