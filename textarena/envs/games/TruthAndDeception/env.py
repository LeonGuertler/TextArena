...
from textarena.envs.games.TruthAndDeception.renderer import create_board_str
    ...
    def _load_facts(self, data_path: Optional[str]) -> None:
        try:
            if data_path is not None: # Use provided path
                ...
                with importlib.resources.files('textarena.envs.games.TruthAndDeception').joinpath('facts.json').open('r') as file:
                    self.facts_data = json.load(file)
