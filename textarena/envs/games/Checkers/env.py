...
from textarena.envs.games.Checkers.renderer import create_board_str
...
class CheckersEnv(ta.Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None):
        ...
        game_state = {"board": self._initialize_board()}
        self.state.reset(game_state=game_state, player_prompt_function=self._generate_player_prompt, role_mapping={0:"Red", 1:"Black"})
    ...
    def _generate_player_prompt(self, player_id: int, game_state: Dict[str, Any]) -> str:
        return (
            f"You are Player {player_id} playing as {'Red' if player_id==0 else 'Black'}.\n"
    ...
    def _execute_player_move(self, action: str):
        ...
        if self._is_valid_move(self.state.current_player_id, row_from, col_from, row_to, col_to):
            ...
            self.state.set_invalid_move(reason=f"Move [{row_from} {col_from} {row_to} {col_to}] is illegal.")
    ...
    def _is_valid_move(self, player_id: int, r1: int, c1: int, r2: int, c2: int) -> bool:
        ...
        if player_id == 0: # Check piece ownership
            ...
            if piece not in ['r', 'R']: return False
        else:
            # Player 1 -> must move 'b' or 'B'
            if piece not in ['b', 'B']: return False
    ...
    def _move_piece(self, player_id: int, r1: int, c1: int, r2: int, c2: int):
        ...
        # Check for kinging
        if piece == 'r' and r2 == 0:
            self.state.game_state["board"][r2][c2] = 'R'  # Red becomes King
        elif piece == 'b' and r2 == 7:
            self.state.game_state["board"][r2][c2] = 'B'  # Black becomes King
    ...
    def _check_gameover(self):
        """ Check if the game is over by seeing if one side has no pieces or no moves. If so, declare a winner or set draw accordingly. """
        ...
        if self.state.check_turn_limit(): # check draw
            self.state.set_draw(reason="The turn limit has been reached.")

    def _has_legal_move(self, player_id: int) -> bool:
        """ Basic check to see if the given player has at least one possible move in this simplified scenario (forward or capturing) """
        for r in range(8):
            for c in range(8):
    ...
    def _can_piece_move(self, r: int, c: int) -> bool:
        ...
        if piece in ['R', 'B']: # Determine directions
            directions = [(-1, -1), (-1, 1), (1, -1), (1, 1)] # King can move in any diagonal direction
        else:
            directions = [(-1, -1), (-1, 1)] if piece=="r" else [(1, -1), (1, 1)] # Normal piece moves (Red goes "up", Black goes "down")
